...
{
    public class SimpleFixedDistributedLoad : BaseModel
    {
        ...
        public SimpleFixedDistributedLoad()
        {
            this.DistributedLoad = 100;
            this.LeftSpan = 0.813;
            this.RightSpan = 0.813;
            this.L = 6;
            this.DeflectionCriteria = 260;
            this.MaxStress = 355;
        }

        public override void Calculate()
        {
            double distributedLoad = DistributedLoad * 1e-2;// N/mm^2
            double l = L * 1e3; // mm
            ...
            double ix = load * Math.Pow(l, 4) / (185 * e * fr); // mm^4
            this.Ix = ix * 1e-4; // cm^4

            double momentAtSupport = load * Math.Pow(l, 2) / 8; ; // N.mm
            double momentAtSpan = 9 *load * Math.Pow(l, 2) / 128; ; // N.mm
            ...
            this.parameters = new object[] { ix, Math.Max(wMinAtSupport, wMinAtSpan) };
        }
